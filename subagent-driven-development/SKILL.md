...
**When dispatching subagents, use the following prompt templates from this skill directory:**

- `./implementer-prompt.md` - Dispatch implementer subagent with task context
- `./spec-reviewer-prompt.md` - Dispatch spec compliance reviewer subagent to verify code matches spec
- `./code-quality-reviewer-prompt.md` - Dispatch code quality reviewer subagent for code quality assessment

**How to use:** Read the prompt template file, substitute placeholders with actual content (e.g., [TASK_TEXT], [SPEC_FILE_PATH], [CODE_CHANGES]), and dispatch to subagent.
